...
architecture IMP of user_logic is
  ...
  type BURST_ST_T is (BST_IDLE, BST_WAIT, BST_READ, BST_WRITE);
  signal burst_state : BURST_ST_T;
  signal addr_in	 :unsigned(31 downto 0);
  signal addr_out	 :unsigned(31 downto 0);
  signal length		 :unsigned(31 downto 0);
  signal start		 :std_logic;
  signal myInterrupt : std_logic;
	...
	if (Bus2IP_Resetn = '0') then
		...
		case burst_state is
			when BST_IDLE =>
				...
				length <= unsigned(slv_reg2);
				if ( mst_go = '1' ) then
					burst_state <= BST_READ;
				end if;
			when BST_READ =>
				if (length > 0 and mst_cmd_sm_busy = '0') then
					mst_ip2bus_addr <= addr_in;
					addr_in <= addr_in + 4;
					mst_cntl_rd_req <= 1;
					start <= '1';
					burst_state <= BST_WAIT;
			...
			when BST_WAIT =>
				...
				mst_cntl_rd_req <= mst_cntl_rd_req;
				mst_cntl_wr_req <= mst_cntl_wr_req;
				if (mst_cmd_sm_state = 
			when BST_WRITE => 
				mst_ip2bus_addr <= addr_out;
				addr_out <= addr_out + 4;
				mst_cntl_wr_req <= 1;
				start <= '1';
				length <= length - 1;
